...
class AgentProvider with ChangeNotifier {
  ...
  Future<void> loadAgents({bool forceRefresh = true}) async {
    ...
    try {
      // FORCER LE RECHARGEMENT POUR AVOIR LES 98 AGENTS
      _agents = await _apiService.getAgents(forceRefresh: true);
      _filteredAgents = List.from(_agents);
      _error = null; // Clear any previous errors

      if (kDebugMode) {
        print('🎯 ${_agents.length} agents chargés dans le provider');
      }
      ...
      // En cas d'erreur, afficher un message d'erreur explicite
      _agents = [];
      _filteredAgents = [];
    } finally {
      _isLoading = false;
