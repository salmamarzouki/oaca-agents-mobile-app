...
class ApiService {
  ...
  Future<List<Agent>> getAgents({bool forceRefresh = false}) async {
    ...
    try {
      // print('🌐 Tentative de connexion à l\'API Django pour 89 agents...');
      ...
      // print('⚠️ API Django inaccessible: $e');
      // print('📂 Utilisation des données locales comme fallback...');

      // Fallback vers les données locales (15 agents)
  ...
  Future<List<Agent>> _fetchFromDjangoAPI() async {
    ...
    if (response.statusCode == 200) {
      ...
      return agentsJson.map((json) => Agent(
        ...
        formation: json['formation'],  // ✅ AJOUT DU CHAMP FORMATION
        formationAutre: json['formation_autre'],  // ✅ AJOUT DU CHAMP FORMATION_AUTRE
  ...
  Future<Map<String, dynamic>> getStats({bool forceRefresh = false}) async {
    ...
    try {
      ...
      ).timeout(const Duration(seconds: 2)); // Timeout très court pour les stats
      ...
      // Si on a un cache, l'utiliser même s'il est expiré
